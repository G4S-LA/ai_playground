...
def main():
    ...
    with TemporaryDirectory() as directory:
        ...
        with sync_playwright() as p:
            ...
            summary_limit = page.locator("#summary-max-tokens")
            expect(summary_limit).to_have_value("512")
            page.locator("#keep-recent").fill("2")
            page.locator("#summary-every").fill("2")

            def send(text, count, expected_errors=0):
                question.fill(text)
                page.locator("#send-button").click()
                expect(page.locator(".message--agent:not(.message--pending)")).to_have_count(count)
                expect(question).to_be_enabled()
                expect(page.locator(".message--error")).to_have_count(expected_errors)

            send("Моё кодовое слово: маяк", 1)
            send("Обсудим проект", 2)
            expect(page.locator("#compression-status")).to_contain_text("сожмём ещё 2")
            summary_limit.fill("0")
            expect(page.locator("#token-preview")).to_contain_text("Лимит summary должен быть положительным")
            summary_limit.fill("1")
            question.fill("Какое моё кодовое слово?")
            page.locator("#send-button").click()
            expect(page.locator(".message--error")).to_contain_text("Summary превысило лимит")
            expect(question).to_be_enabled()
            expect(question).to_have_value("Какое моё кодовое слово?")
            expect(page.locator(".message--user")).to_have_count(2)
            summary_limit.fill("1024")
            send("Какое моё кодовое слово?", 3, expected_errors=1)
            ...
            expect(summary_limit).to_have_value("1024")
            ...
            browser.close()
            print("Browser OK: сжатие, изменение лимита summary после ошибки, on/off, перезагрузка, расход, mobile.")
