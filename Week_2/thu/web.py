...
class WebAgent(Protocol):
    def set_compression(self, **changes) -> None:
        ...
...
def create_app(
    ...
    # Локальный учебный сервер: настройка окна и ход выполняются атомарно.
    agent_lock = RLock()
    ...
    def get_messages(chat_id: str):
        ...
        return jsonify(
            ...
            statistics=current_agent(chat_id).statistics(),
        )

    def configured_agent(chat_id: str, payload: dict) -> WebAgent:
        agent = current_agent(chat_id)
        tokens = payload.get("context_window_tokens")
        if "context_window_tokens" in payload and (type(tokens) is not int or tokens <= 0):
            raise AgentError("Размер окна должен быть положительным целым числом")
        if "compression" in payload:
            if not isinstance(payload["compression"], dict):
                raise AgentError("Настройки сжатия должны быть объектом")
            agent.set_compression(**payload["compression"])
        if "context_window_tokens" in payload:
            agent.set_context_window(tokens)
        return agent

    @app.post("/api/chats/<chat_id>/preview")
    def preview_message(chat_id: str):
        if existing_chat(chat_id) is None:
            return jsonify(error="Чат не найден"), 404
        payload = request.get_json(silent=True)
        if not isinstance(payload, dict) or not isinstance(payload.get("message"), str):
            return jsonify(error="Сообщение должно быть строкой"), 400
        if len(payload["message"]) > MAX_MESSAGE_LENGTH:
            return jsonify(error="Сообщение слишком длинное"), 400
        with agent_lock:
            try:
                agent = configured_agent(chat_id, payload)
                return jsonify(preview=agent.preview(payload["message"]))
    ...
    def send_message(chat_id: str):
        ...
        with agent_lock:
            ...
            try:
                ...
                return jsonify(
                    error=str(error), code="context_overflow", preview=error.preview,
                    statistics=agent.statistics(),
                ), 413
            except AgentError as error:
                return jsonify(error=str(error), statistics=agent.statistics()), 502

        updated_chat = existing_chat(chat_id)
        if updated_chat is None:
            return jsonify(error="Чат не найден после сохранения ответа"), 500
        return jsonify(
            answer=answer, chat=_chat_to_dict(updated_chat), statistics=statistics,
        )
    ...
    def agent_error(error: AgentError):
        return jsonify(error=str(error)), 500
