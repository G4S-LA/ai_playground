...
class ChatRepository:
    ...
    def load_context(self, chat_id: str) -> dict | None:
        try:
            with closing(self._connect()) as connection:
            ...
            raise ChatRepositoryError(f"Не удалось прочитать summary: {error}") from error

    def save_compression_settings(self, chat_id: str, settings: dict, *, reset_summary: bool = False) -> None:
        try:
            with closing(self._connect()) as connection, connection:
                connection.execute(
                    """INSERT INTO chat_context(chat_id, settings_json) VALUES (?, ?)
                       ON CONFLICT(chat_id) DO UPDATE SET settings_json = excluded.settings_json""",
                    (chat_id, json.dumps(settings)),
                )
                if reset_summary:
                    connection.execute(
                        "UPDATE chat_context SET summary_text = '', summarized_messages = 0 WHERE chat_id = ?",
                        (chat_id,),
                    )
        except sqlite3.Error as error:
            raise ChatRepositoryError(f"Не удалось сохранить настройки: {error}") from error

    def save_summary(
        self, chat_id: str, summary: str, covered: int, previous_covered: int,
        settings: dict, stats: dict,
    ) -> None:
        """Summary, граница обработанной истории и расход записываются атомарно."""
        try:
            with closing(self._connect()) as connection, connection:
                connection.execute("BEGIN IMMEDIATE")
                row = connection.execute(
                    "SELECT summarized_messages FROM chat_context WHERE chat_id = ?", (chat_id,),
                ).fetchone()
                if (row[0] if row else 0) != previous_covered:
                    raise ChatRepositoryError("Summary уже обновлён другим экземпляром. Повторите запрос.")
                connection.execute(
                    """INSERT INTO chat_context(chat_id, settings_json, summary_text, summarized_messages)
                       VALUES (?, ?, ?, ?) ON CONFLICT(chat_id) DO UPDATE SET
                       summary_text = excluded.summary_text,
                       summarized_messages = excluded.summarized_messages""",
                    (chat_id, json.dumps(settings), summary, covered),
                )
                connection.execute(
                    "INSERT INTO summary_calls(chat_id, stats_json) VALUES (?, ?)",
                    (chat_id, json.dumps(stats, ensure_ascii=False, allow_nan=False)),
                )
        except sqlite3.Error as error:
            raise ChatRepositoryError(f"Не удалось сохранить summary: {error}") from error

    def load_summary_calls(self, chat_id: str) -> list[dict]:
        try:
            with closing(self._connect()) as connection:
                rows = connection.execute(
                    "SELECT stats_json FROM summary_calls WHERE chat_id = ? ORDER BY id", (chat_id,),
                ).fetchall()
            return [json.loads(row["stats_json"]) for row in rows]
        except (sqlite3.Error, ValueError) as error:
            raise ChatRepositoryError(f"Не удалось прочитать расход сжатия: {error}") from error
