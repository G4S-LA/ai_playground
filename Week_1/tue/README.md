...
1. без ограничений;
2. только строгий формат через `response_format`;
...
Обычный структурированный ответ содержит поле `answer`. В профиле со всеми
ограничениями дополнительно требуется поле:
...
`answer`, затем установить `completion_status` в `completed` и закрыть JSON.
