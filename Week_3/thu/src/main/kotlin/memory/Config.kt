    ...
    companion object {
        fun fromEnvironment(
            ...
            require(demo || apiKey.isNotBlank()) {
                "Не задана переменная LLM_API_KEY или DASHSCOPE_API_KEY в окружении или файле .env. " +
            ...
            return AppConfig(
                ...
                model = if (demo) "demo-invariant-model" else setting("LLM_MODEL") ?: "gpt-4o-mini",
                systemPrompt = setting("LLM_SYSTEM_PROMPT")
                    ?: "Ты — полезный ассистент. Соблюдай переданные инварианты и используй память как контекст.",
