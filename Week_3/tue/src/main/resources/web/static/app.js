...
function setBusy(value) {
  ...
  for (const element of [
    ...
    $("#profile-select"), $("#new-profile"), $("#profile-name"),
    $("#profile-style"), $("#profile-format"), $("#profile-constraints"),
    $("#save-profile"), $("#delete-profile"),
  ]) element.disabled = value || !currentSessionId;
  if (!value && currentSnapshot) {
    $("#delete-profile").disabled = currentSnapshot.profile.id === "default";
  }
...
function renderProfile(profile) {
  ...
  $("#profile-style").value = profile.style;
  $("#profile-format").value = profile.format;
  $("#profile-constraints").value = profile.constraints.join("\n");
  $("#delete-profile").disabled = busy || profile.id === "default";
}

async function refreshProfiles(selectedId = currentSnapshot?.profile.id) {
  profiles = (await requestJson("/api/profiles")).profiles;
...
$("#profile-select").addEventListener("change", () => runAction(async () => {
  const result = await requestJson(
    sessionUrl("/profile"),
    jsonOptions("PUT", {profileId: $("#profile-select").value}),
  );
  renderSnapshot(result.snapshot);
  $("#profile-status").textContent = "Профиль подключён к текущему диалогу.";
  await refreshSessions();
}));
...
$("#new-profile").addEventListener("click", () => {
  ...
  $("#profile-status").textContent = "Заполните новый профиль.";
  $("#profile-name").focus();
});
...
$("#profile-form").addEventListener("submit", (event) => {
  ...
  runAction(async () => {
    const body = {
      ...
      format: $("#profile-format").value.trim(),
    ...
    const saved = editingProfileId
      ? await requestJson(`/api/profiles/${encodeURIComponent(editingProfileId)}`, jsonOptions("PUT", body))
      : await requestJson("/api/profiles", jsonOptions("POST", body));
    await refreshProfiles(saved.profile.id);
    const result = await requestJson(
      sessionUrl("/profile"),
      jsonOptions("PUT", {profileId: saved.profile.id}),
    );
    renderSnapshot(result.snapshot);
    $("#profile-status").textContent = "Профиль сохранён и применяется к каждому запросу.";
  });
});

$("#delete-profile").addEventListener("click", () => {
  if (busy || !editingProfileId || editingProfileId === "default") return;
  if (!window.confirm("Удалить этот профиль? Связанные диалоги перейдут на нейтральный.")) return;
  runAction(async () => {
    await requestJson(`/api/profiles/${encodeURIComponent(editingProfileId)}`, {method: "DELETE"});
    await refreshProfiles("default");
    await selectSession(currentSessionId);
    $("#profile-status").textContent = "Профиль удалён; подключён нейтральный профиль.";
  });
});
...
$("#chat-form").addEventListener("submit", (event) => {
  ...
  runAction(async () => {
    ...
    const pending = addMessage("agent", "Модель применяет профиль и собирает prompt из памяти…");
...
$("#new-session").addEventListener("click", () => runAction(async () => {
  const result = await requestJson(
    "/api/sessions",
    jsonOptions("POST", {profileId: currentSnapshot.profile.id}),
  );
...
$("#delete-session").addEventListener("click", () => {
  ...
  runAction(async () => {
    const profileId = currentSnapshot.profile.id;
    await requestJson(sessionUrl(), {method: "DELETE"});
    await refreshSessions();
    const next = knownSessions[0] || (await requestJson(
      "/api/sessions",
      jsonOptions("POST", {profileId}),
    )).session;
...
runAction(async () => {
  ...
  await refreshSessions();
  const initial = knownSessions[0] || (await requestJson(
    "/api/sessions",
    jsonOptions("POST", {profileId: "default"}),
  )).session;
