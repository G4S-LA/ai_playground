...
class DemoLanguageModel : LanguageModel {
    override fun complete(messages: List<PromptMessage>): String {
        ...
        val profile = messages.firstOrNull { it.content.startsWith("[USER_PROFILE]") }?.content.orEmpty()
        val profileName = profile.field("name").ifEmpty { "Неизвестный" }
        val style = profile.field("preferred_style").ifEmpty { "не задан" }
        val format = profile.field("preferred_format").ifEmpty { "не задан" }
        ...
        val workingCount = messages.firstOrNull { it.content.startsWith("[WORKING_MEMORY]") }
            ?.content?.lineSequence()?.count { it.startsWith("- ") } ?: 0
        return "Профиль «$profileName» · стиль: $style · формат: $format. " +
            "Демо-ответ на «${query.take(120)}». " +
