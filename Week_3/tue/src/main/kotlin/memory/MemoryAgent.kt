...
class MemoryAgent(
    ...
    fun createSession(profileId: String = UserProfiles.DEFAULT_ID): SessionSummary =
        store.createSession(profileId)

    fun listProfiles(): List<UserProfile> = store.listProfiles()

    fun createProfile(request: ProfileRequest): UserProfile = store.createProfile(
        request.name.orEmpty(),
        request.style.orEmpty(),
        request.format.orEmpty(),
        request.constraints.orEmpty(),
    )

    fun updateProfile(profileId: String, request: ProfileRequest): UserProfile = store.updateProfile(
        profileId,
        request.name.orEmpty(),
        request.style.orEmpty(),
        request.format.orEmpty(),
    ...
    fun selectProfile(sessionId: String, profileId: String): AgentSnapshot {
        store.selectProfile(sessionId, profileId)
        return snapshot(sessionId)
    }

    fun deleteProfile(profileId: String) = store.deleteProfile(profileId)
    ...
    private fun MemorySnapshot.withModel() = AgentSnapshot(session, working, longTerm, profile, modelName)
}
