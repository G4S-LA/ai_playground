...
class MemoryCli(private val agent: MemoryAgent) {
    ...
    fun run() {
        println("Агент с памятью и формальным состоянием задачи. /help — список команд.")
        printSession()
        while (true) {
            print("Вы: ")
    ...
    private fun printHelp() {
        println(
            ...
            Для сложного запроса: planning → execution → validation → done.
            Для простого: execution → validation → done. Каждый этап печатается в момент начала.
            """.trimIndent()
        )
    }
}
