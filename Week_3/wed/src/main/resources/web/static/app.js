...
function renderTaskState(state) {
  ...
  for (const node of document.querySelectorAll(".state-flow [data-stage]")) {
    ...
    node.classList.toggle("is-complete", index < activeIndex && !planningSkipped);
  ...
  $("#state-status").textContent = state.stage === "idle"
    ? "Введите запрос: агент определит, нужен ли этап planning."
    : state.stage === "done"
      ? "Цикл завершён. Новый запрос начнётся с planning или execution."
      : "Сейчас агент действительно выполняет выделенный этап.";
}
