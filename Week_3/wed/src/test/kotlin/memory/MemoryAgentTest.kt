...
class MemoryAgentTest {
    ...
    @Test
    fun `repeated validation rejection returns clarification instead of error`() {
        val model = RecordingModel(
            "Неподходящий ответ",
            "REVISE\nЗапрос непонятен",
        ...
        assertEquals("done", response.snapshot.taskState.stage)
        assertEquals("Подготовлен запрос на уточнение", response.snapshot.taskState.expectedAction)
        assertEquals(
            listOf("execution", "validation", "execution", "validation", "done"),
            response.snapshot.session.messages.last().stateTrace.map { it.stage },
        )
    ...
    fun `failed model call does not add an unfinished turn`() {
        ...
        assertEquals("execution", agent.snapshot(session.id).taskState.stage)
    }
